...
public class PantallaProducto extends AppCompatActivity {
    ...
    /**
     * Carga el producto recibido por Intent
     */
    private void configurarProducto() {

        Producto producto =
                (Producto) getIntent().getSerializableExtra("producto");

        if (producto != null) {
            recyclerProductoDetalle.setAdapter(
                    new ProductoDetalleAdapter(producto)
            );
        }
    }
